...
For biological data: * To be completed *
...
Once data has been formatted into .rda files (contained in `data-raw` in the main package directory), it is ready to be combined into the final package data tables. This can be done be running the function `combine_all_data()`. This function requires no arguments and results in final .rda files being saved in `data`. This function should be used with caution. Please ensure that you have followed the above steps in order and that the `data-raw` folder has been updated before running `combine_all_data()`. The output from this function is visible in the main package directory in `data` as .rda files organized into data tables by temporal, spatial and categorical scales. 
