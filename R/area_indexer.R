...
area_indexer <- function(years = NULL, areanames = NULL, areaTypes = NULL, datafiles = NULL, quiet = F){
  ...
  warnanator <- function(param = NULL, fail=F){
    ...
    if (fail) {
      ...
      message("One or more of value(s) for ",param, " was not found in the data, ",paste(res))
    }
  }
  ...
  if(length(years)>0){
    if (!any(years %in% area_year_df$year)) warnanator("year", fail = T)
    if (!all(years %in% area_year_df$year)) warnanator("year", fail = F)
    area_year_df <- area_year_df[area_year_df$year %in% years,]
  }

  if(length(areanames)>0){
    if (!any(areanames %in% toupper(area_year_df$areaname))) warnanator("areaname", fail = T)
    if (!all(areanames %in% toupper(area_year_df$areaname))) warnanator("areaname", fail = F)
    area_year_df <- area_year_df[toupper(area_year_df$areaname) %in% areanames,]
  }
  if(length(areaTypes)>0){
    if (!any(areaTypes %in% toupper(area_year_df$areaType))) warnanator("areaType", fail = T)
    if (!all(areaTypes %in% toupper(area_year_df$areaType))) warnanator("areaType", fail = F)
    area_year_df <- area_year_df[toupper(area_year_df$areaType) %in% areaTypes,]
  }

  if(length(datafiles)>0){
    if (!any(datafiles %in% toupper(area_year_df$dataframe))) warnanator("dataframe", fail = T)
    if (!all(datafiles %in% toupper(area_year_df$dataframe))) warnanator("dataframe", fail = F)
