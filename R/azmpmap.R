...
azmpmap <- function(){
  ...
  AZMP_sf = df2sf(input = AZMP, PID = "record", type.field = "type", ORD = "vertice", point.IDs = c("station"), line.IDs = "section", poly.IDs = c("nafo","area"))
  ...
  .leaflet-control.map-title {
    ...
    font-weight: bold;
    font-size: 28px;'>AZMP multivaR Map </div>")
  ...
  m <- leaflet::addCircles(map = m, data = AZMP_sf[AZMP_sf$type=="station",],
                           group= "Stations", label =~sname,
                           color = "red",
                           labelOptions = leaflet::labelOptions(noHide = T, textOnly = TRUE),
                           popup = ~paste0("Station:", lname,"<br>Depth: ", depth))
  m <- leaflet::addPolylines(map = m, data = AZMP_sf[AZMP_sf$type=="section",],
                             group= "Sections", label = ~sname,
                             color="red", weight = 1.5,
                             popup = ~paste0("Section:", lname))
